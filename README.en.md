...
| `--trust` | flag | `false` | Record every currently-TRUSTED skill's scope into `~/.skillsig/lock.yaml` as the drift baseline (run once when you first trust a corpus). A skill that has DRIFTED vs. the baseline is **not** recorded — it reports `SCOPE-DRIFTED` and keeps its old baseline; use `--force-trust` to re-baseline it on purpose |
| `--force-trust` | flag | `false` | With `--trust`, also re-baseline a `SCOPE-DRIFTED` skill (overwrite its lock entry with the broadened scope). Off by default so `--trust` can't silently launder a scope escalation into the lock |
...
- [x] **v0.5.0** — closes the last un-boundaried axis: **tools**. A wildcard `Bash(git status*)` used a raw prefix, so a look-alike **sibling** command `Bash(git statuscheckout --force)` (a different subcommand that merely shares the text prefix) counted as covered — letting a re-signed skill swap one same-prefix subcommand for another and slip past `SCOPE-DRIFTED` on both `diff` and `verify --ci`. Wildcard matching is now **argument-token boundary-aware** (the extra text must begin a new space-delimited token), matching the `/` and `.` boundaries used by `fs_write` / `network_egress`: refinements like `git status -s` / `--porcelain` stay covered, the look-alike sibling is flagged growth. All four declared axes (model × tools × fs_write × network_egress) now share one definition of "broader"
- [x] **v0.6.0** — two verification-correctness fixes. ① **`verify` actually checks the signature bundle now**: `runVerify` never called the `verifier` package — `verifier.VerifySkill` (canonicalize manifest, locate `skillsig.bundle`, verify the ed25519 signature) was fully implemented but had **zero callers**, so a scope-clean skill with **no bundle**, or with a bundle that didn't match its manifest, still read `TRUSTED`. An attacker who tampered a manifest to cover a malicious grant and shipped no/invalid bundle passed `verify --ci` `TRUSTED` at cold-start (before any lock baseline exists) — the exact supply-chain vector skillsig exists to catch. `verify` now folds signature verification into every row: missing bundle downgrades `TRUSTED` → `UNSIGNED`, a bundle that doesn't verify (tampered/forged) → `SCOPE-DRIFTED` (and fails `--ci`), keyless bundles surface as pending, and `TRUSTED` requires a valid signature. ② **`verify --trust` no longer silently re-baselines a drifted scope**: `ScanAndTrust` ran only the in-version check and recorded the *current* (possibly broadened) declares into the lock, so a skill that quietly broadened its scope got re-baselined and printed `TRUSTED`, erasing the drift. `--trust` now runs the cross-version lock-drift check first: a drifted skill is `SCOPE-DRIFTED` and its baseline is preserved (a later plain `verify` still catches it); re-baselining a drifted skill on purpose requires the explicit `verify --trust --force-trust`
- [x] **v0.7.0** — closes the completing gap the v0.6.0 fixes missed: v0.6.0 made a `TRUSTED` verdict require a valid signature in what `verify` **displays** (and fails on in `--ci`), but the `verify --trust` **recording** path still decided what to pin into `~/.skillsig/lock.yaml` from the scope check alone — which knows nothing about signatures. So a skill with a manifest and matching scope but **no signature bundle** showed `UNSIGNED` yet still had its declared scope silently pinned into the lock, contradicting the "you only pin scopes you actually vouch for" contract and re-opening a laundering path (once that never-vouched skill later ships any valid bundle, its broad scope matches the pre-seeded baseline and reads fully `TRUSTED` with no drift). `verify --trust` now gates recording on the signature verdict — it pins **only** `SIGNED` / `KEYLESS-PENDING` skills, exactly the set `verify` shows as `TRUSTED`, so `NO-BUNDLE` and `BAD-SIGNATURE` skills are never recorded (on both the clean-`TRUSTED` and `--force-trust` paths)
